export const dummymatches={
    upper: [
      {
        id: 20231,
        name: 'UB 1.10',
        nextMatchId: 20242,
        nextLooserMatchId: 20257,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: '5a2188a4-5e8b-4489-96ba-0140b4342cfd',
            resultText: null,
            isWinner: false,
            status: null,
            name:"larry"
          },
        ],
      },
      {
        id: 20232,
        name: 'UB 2.8',
        nextMatchId: 20210,
        nextLooserMatchId: 20217,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20233,
        name: 'UB 2.7',
        nextMatchId: 20210,
        nextLooserMatchId: 20220,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '8cfbe1b3-090e-4665-82bb-aff37bebb6f4',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20234,
        name: 'UB 2.4',
        nextMatchId: 20207,
        nextLooserMatchId: 20221,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '2ff0c36b-3acc-4c21-90c4-6eeec34e7828',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20235,
        name: 'UB 4.2',
        nextMatchId: 20214,
        nextLooserMatchId: 20223,
        tournamentRoundText: 'UB 4',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20236,
        name: 'UB 1.14',
        nextMatchId: 20233,
        nextLooserMatchId: 20261,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: '8cfbe1b3-090e-4665-82bb-aff37bebb6f4',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20237,
        name: 'UB 4.1',
        nextMatchId: 20214,
        nextLooserMatchId: 20229,
        tournamentRoundText: 'UB 4',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20238,
        name: 'UB 2.3',
        nextMatchId: 20207,
        nextLooserMatchId: 20227,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '781e48d6-2c4d-496d-92e8-6385ea9a7782',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20239,
        name: 'UB 1.13',
        nextMatchId: 20233,
        nextLooserMatchId: 20261,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: 'c08a1b93-ada2-45b2-8825-f268f167b040',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: 'b5cd08d2-1596-4c08-9851-75b4f4de7f60',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20240,
        name: 'UB 2.6',
        nextMatchId: 20205,
        nextLooserMatchId: 20218,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20241,
        name: 'UB 1.12',
        nextMatchId: 20240,
        nextLooserMatchId: 20254,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '0ea06043-7293-4fff-a147-9e47514b6156',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: '3f7494da-29bb-4594-8a29-7a73080521b5',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20242,
        name: 'UB 2.5',
        nextMatchId: 20205,
        nextLooserMatchId: 20224,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '5a2188a4-5e8b-4489-96ba-0140b4342cfd',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20243,
        name: 'UB 1.11',
        nextMatchId: 20240,
        nextLooserMatchId: 20254,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: 'a601f9ee-1efd-4542-94ab-e62beb690884',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: '0df5a720-27cd-4ab7-a9b2-1682ac4efe3d',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20244,
        name: 'UB 2.2',
        nextMatchId: 20212,
        nextLooserMatchId: 20226,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: 'ea7cc04f-aa82-4072-a7c8-397ea3badd6b',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: '6a602af0-5425-46fd-a3eb-f4d4559717d4',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20245,
        name: 'UB 1.16',
        nextMatchId: 20232,
        nextLooserMatchId: 20260,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: 'd9edef7d-747f-4516-8dc0-667a2d923785',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: 'c3116829-63bf-4dfb-8b77-9925eea7a988',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20246,
        name: 'UB 2.1',
        nextMatchId: 20212,
        nextLooserMatchId: 20230,
        tournamentRoundText: 'UB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '59c63115-d023-46b7-803b-c95d2b03c802',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: 'ea43f65a-81ab-4aea-a945-c7bda5ce87e6',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20247,
        name: 'UB 1.15',
        nextMatchId: 20232,
        nextLooserMatchId: 20260,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '61da8bf1-9e6f-4435-b8f5-662c3e28b771',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: 'e37b4d05-d5c6-4561-bac6-9c8c9a0611cd',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20263,
        name: 'Final',
        nextMatchId: null,
        nextLooserMatchId: 1,
        tournamentRoundText: 'UB 6',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20202,
        name: 'UB 1.9',
        nextMatchId: 20242,
        nextLooserMatchId: 20257,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '6c16a503-6577-4f67-9264-935b4dcd5247',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: '2f9dddd8-eb61-4d73-a34b-a5ba7c6c8311',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20203,
        name: 'UB 1.8',
        nextMatchId: 20234,
        nextLooserMatchId: 20250,
        tournamentRoundText: 'UB 1',
        startTime:"",
        state: 'SCHEDULED',
        participants: [
          {
            id: '8d4037de-aeb0-4c04-a0b4-868560561d78',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: '827f34e6-9964-4038-b4a4-663016f18f02',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20204,
        name: 'UB 1.5',
        nextMatchId: 20238,
        nextLooserMatchId: 20252,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          {
            id: '0b2ce314-e4ed-4abf-9050-69d51a45e0d7',
            resultText: null,
            isWinner: false,
            status: null,
          },
          {
            id: '7c44871a-fbdd-4861-a690-600ddb427d51',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20205,
        name: 'UB 3.3',
        nextMatchId: 20235,
        nextLooserMatchId: 20222,
        tournamentRoundText: 'UB 3',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20206,
        name: 'UB 1.4',
        nextMatchId: 20244,
        nextLooserMatchId: 20248,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: '6a602af0-5425-46fd-a3eb-f4d4559717d4',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20207,
        name: 'UB 3.2',
        nextMatchId: 20237,
        nextLooserMatchId: 20225,
        tournamentRoundText: 'UB 3',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20208,
        name: 'UB 1.7',
        nextMatchId: 20234,
        nextLooserMatchId: 20250,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: '2ff0c36b-3acc-4c21-90c4-6eeec34e7828',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20209,
        name: 'UB 1.6',
        nextMatchId: 20238,
        nextLooserMatchId: 20252,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: '781e48d6-2c4d-496d-92e8-6385ea9a7782',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20210,
        name: 'UB 3.4',
        nextMatchId: 20235,
        nextLooserMatchId: 20228,
        tournamentRoundText: 'UB 3',
        startTime: '',
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20211,
        name: 'UB 1.1',
        nextMatchId: 20246,
        nextLooserMatchId: 20249,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: '59c63115-d023-46b7-803b-c95d2b03c802',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20212,
        name: 'UB 3.1',
        nextMatchId: 20237,
        nextLooserMatchId: 20219,
        tournamentRoundText: 'UB 3',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20213,
        name: 'UB 1.3',
        nextMatchId: 20244,
        nextLooserMatchId: 20248,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: 'ea7cc04f-aa82-4072-a7c8-397ea3badd6b',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
      {
        id: 20214,
        name: 'UB Semi Final',
        nextMatchId: 20263,
        nextLooserMatchId: 20216,
        tournamentRoundText: 'UB 5',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20215,
        name: 'UB 1.2',
        nextMatchId: 20246,
        nextLooserMatchId: 20249,
        tournamentRoundText: 'UB 1',
        startTime: "",
        state: 'WALK_OVER',
        participants: [
          {
            id: 'ea43f65a-81ab-4aea-a945-c7bda5ce87e6',
            resultText: null,
            isWinner: false,
            status: null,
          },
        ],
      },
    ],
    lower: [
      {
        id: 20224,
        name: 'LB 2.4',
        nextMatchId: 20251,
        nextLooserMatchId: 2,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20225,
        name: 'LB 4.2',
        nextMatchId: 20259,
        nextLooserMatchId: 4,
        tournamentRoundText: 'LB 4',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20226,
        name: 'LB 2.7',
        nextMatchId: 20255,
        nextLooserMatchId: 5,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20227,
        name: 'LB 2.6',
        nextMatchId: 20258,
        nextLooserMatchId: 6,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20228,
        name: 'LB 4.4',
        nextMatchId: 20256,
        nextLooserMatchId: 7,
        tournamentRoundText: 'LB 4',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20229,
        name: 'LB 6.2',
        nextMatchId: 20262,
        nextLooserMatchId: 8,
        tournamentRoundText: 'LB 6',
        startTime: '',
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20230,
        name: 'LB 2.8',
        nextMatchId: 20255,
        nextLooserMatchId: 11,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20248,
        name: 'LB 1.2',
        nextMatchId: 20220,
        nextLooserMatchId: 12,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          { id: "pee", resultText: null, isWinner: false, status: 'NO_PARTY' },
          { id: "pee", resultText: null, isWinner: false, status: 'NO_PARTY' },
        ],
      },
      {
        id: 20249,
        name: 'LB 1.1',
        nextMatchId: 20217,
        nextLooserMatchId: 111,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          { id: "gaga", resultText: null, isWinner: false, status: 'NO_PARTY' },
          { id: "gogo", resultText: null, isWinner: false, status: 'NO_PARTY' },
        ],
      },
      {
        id: 20250,
        name: 'LB 1.4',
        nextMatchId: 20224,
        nextLooserMatchId: 12300,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          { id: "idklol", resultText: null, isWinner: false, status: 'NO_PARTY' },
        ],
      },
      {
        id: 20251,
        name: 'LB 3.2',
        nextMatchId: 20225,
        nextLooserMatchId: 321,
        tournamentRoundText: 'LB 3',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20252,
        name: 'LB 1.3',
        nextMatchId: 20218,
        nextLooserMatchId: 19,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          { id: "harder", resultText: null, isWinner: false, status: 'NO_PARTY' },
        ],
      },
      {
        id: 20253,
        name: 'LB 3.1',
        nextMatchId: 20219,
        nextLooserMatchId: 291,
        tournamentRoundText: 'LB 3',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20254,
        name: 'LB 1.6',
        nextMatchId: 20227,
        nextLooserMatchId: 567,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20255,
        name: 'LB 3.4',
        nextMatchId: 20228,
        nextLooserMatchId: 333222,
        tournamentRoundText: 'LB 3',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20256,
        name: 'LB 5.2',
        nextMatchId: 20229,
        nextLooserMatchId: 987,
        tournamentRoundText: 'LB 5',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20257,
        name: 'LB 1.5',
        nextMatchId: 20221,
        nextLooserMatchId: 678,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          { id: "work", resultText: null, isWinner: false, status: 'NO_PARTY' },
        ],
      },
      {
        id: 20258,
        name: 'LB 3.3',
        nextMatchId: 20222,
        nextLooserMatchId: 101010,
        tournamentRoundText: 'LB 3',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20259,
        name: 'LB 5.1',
        nextMatchId: 20223,
        nextLooserMatchId: 1010,
        tournamentRoundText: 'LB 5',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20260,
        name: 'LB 1.8',
        nextMatchId: 20230,
        nextLooserMatchId: 101,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20261,
        name: 'LB 1.7',
        nextMatchId: 20226,
        nextLooserMatchId: 0,
        tournamentRoundText: 'LB 1',
        startTime: "",
        state: 'SCHEDULED',
        participants: [
          { id: "pp", resultText: null, isWinner: false, status: 'NO_PARTY' },
        ],
      },
      {
        id: 20262,
        name: 'LB 7.1',
        nextMatchId: 20216,
        nextLooserMatchId: 222222,
        tournamentRoundText: 'LB 7',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20216,
        name: 'LB Semi Final',
        nextMatchId: 20263,
        nextLooserMatchId: 121,
        tournamentRoundText: 'LB 8',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20217,
        name: 'LB 2.1',
        nextMatchId: 20253,
        nextLooserMatchId: 333333,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20218,
        name: 'LB 2.3',
        nextMatchId: 20251,
        nextLooserMatchId: 567,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20219,
        name: 'LB 4.1',
        nextMatchId: 20259,
        nextLooserMatchId: 112,
        tournamentRoundText: 'LB 4',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20220,
        name: 'LB 2.2',
        nextMatchId: 20253,
        nextLooserMatchId: 332,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20221,
        name: 'LB 2.5',
        nextMatchId: 20258,
        nextLooserMatchId: 333,
        tournamentRoundText: 'LB 2',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20222,
        name: 'LB 4.3',
        nextMatchId: 20256,
        nextLooserMatchId: 789,
        tournamentRoundText: 'LB 4',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
      {
        id: 20223,
        name: 'LB 6.1',
        nextMatchId: 20262,
        nextLooserMatchId: 23,
        tournamentRoundText: 'LB 6',
        startTime: "",
        state: 'SCHEDULED',
        participants: [],
      },
    ],
  }